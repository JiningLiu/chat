var script = (function() {
  ...
  const docRef = window.location.search.includes('?id=') ? db.collection("chat").doc(window.location.search.replace("?id=", "")) : db.collection("chat").doc('10000000');
  ...
  function resetPublicRooms(date, reportOnly) {
    if (username.includes(" ☆")) {
      if (!reportOnly) {
        db.collection("chat").doc('10000000').set({
          "1": "Public Chat Room - " + date, 
          "1p": "Chat for School"
        }, { merge: false });
      }
      db.collection("chat").doc('69696969').set({
        "1": "Report - " + date, 
        "1p": "Chat for School"
      }, { merge: false });
    }
      ...
      firebase.auth()
        ...
        .then((result) => {
          if (result.user != null) {
            ...
            document.getElementById('indexMain').innerHTML = '<button style="margin-top: 7px; width: 155px;" onclick="script.signIn()">Sign Up</button>    <br>    <button style="margin-top: 7px; width: 155px;" onclick="script.signIn()">Log In</button>'
            document.getElementById('indexFooter').innerHTML = '<p style="padding-top: 10px; font-weight: 500; font-size: 75%; padding-bottom: 10px">v1.1.2 (24) by <a href="//candice" class="aReset" style="text-decoration: none; color: lightgray;">Candice</a></p><a class="aReset" rel="license" href="http://creativecommons.org/licenses/by-nc-sa/4.0/" target="_blank"><img alt="Creative Commons License" style="border-width:0" src="https://i.creativecommons.org/l/by-nc-sa/4.0/88x31.png" /></a>'
          }
        });
    }
  });
  ...
  function loadChat() {
    docRef.onSnapshot((doc) => {
      if (doc.exists && window.location.href.includes('?id=')) {
        ...
        document.getElementById("loading").innerHTML = '<span><img src="logo.png" style="height: 30px; padding-top: 8px; padding-left: 5px; padding-right: 5px; margin: 0; filter: invert(100%);" alt="Chat for School"></span>';
        $('#redirectCheck').fadeIn(0);
        $('#lowpdiv').fadeIn(0);
        var lastItem = 0;
        for(var i = 1; i <= Infinity; i ++) {
          if (document.hidden && "" + doc.data()[i + "p"] != id && "" + doc.data()[i + 1] == "undefined") {
            var notification = new Notification("" + doc.data()[i + "p"], { body: "" + doc.data()[i] });
          }
    ...
    db.collection("user").doc(user.email).get()
      .then((doc) => {
        if (!doc.exists) {
          ...
          $('#username').on('keyup', function (e) {
            ...
            if (e.key === 'Enter' || e.keyCode === 13) {
              db.collection("username").doc('default').get().then((doc) => {
                if (doc.exists && document.getElementById("username").value.toLowerCase().includes("username") == false && document.getElementById("username").value.toLowerCase().includes("invalid") == false && document.getElementById("username").value.toLowerCase().includes("jining") == false && document.getElementById("username").value.toLowerCase().includes("☆") == false && document.getElementById("username").value.toLowerCase() != "all" && document.getElementById("username").value.toLowerCase() != "chat" && alphanumeric(document.getElementById("username").value)) {
                  for(var i = 1; i <= Infinity; i ++) {
          ...
          id = username;
          db.collection('ban').doc(username).get().then((doc) => {
            if (doc.exists && doc.data()['banned']) {
              document.querySelector('html').innerHTML = '<!DOCTYPE html> <html lang="en"> <head> <meta charset="UTF-8"> <meta name="viewport" content="width=device-width, initial-scale=1.0"> <title>CFS - BAN</title> </head> <body> <h1 style="font-size: 69px; margin: 0; padding: 0; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center;">ur banned get good</h1> </body> </html>';
            }
          })
          const date = new Date();
          if (window.location.pathname == '/') {
            document.getElementById('welcome').innerHTML = 'Welcome back, ' + username + '!';
            document.getElementById('indexMain').innerHTML = '<button onclick="script.create()" style="transform: scale(1.25); width: 124px; margin-bottom: 10px;">Create a chat</button>       <br>       <input type="text" id="join" class="join" maxlength="8" placeholder="8-digit ID">       <button onclick="script.join()">Join</button>       <br>       <button style="margin-top: 7px; width: 155px;" onclick="window.location.href = \'./chat.html?id=10000000\'">Public Chat Room</button>       <br>       <button style="margin-top: 7px; width: 155px;" onclick="window.location.href = \'./chat.html?id=69696969\'">Report</button>';   
            document.getElementById('indexFooter').innerHTML = '<button onclick="script.reset()" id="reset">Sign Out</button><p style="padding-top: 10px; font-weight: 500; font-size: 75%; padding-bottom: 10px">v1.1.2 (24) by <a href="//candice" class="aReset" style="text-decoration: none; color: lightgray;">Candice</a></p><a class="aReset" rel="license" href="http://creativecommons.org/licenses/by-nc-sa/4.0/" target="_blank"><img alt="Creative Commons License" style="border-width:0" src="https://i.creativecommons.org/l/by-nc-sa/4.0/88x31.png" /></a>';
  ...
  $('#msg').on('keyup', function (e) {
    if ((e.key === 'Enter' || e.keyCode === 13) && document.getElementById("msg").value != "") {
      if (document.getElementById("msg").value.includes("!cmd/") && id.includes(" ☆")) {
  ...
  function create() {
    ...
    db.collection("chat").doc('' + createdID).set({
  ...
  function join() {
    if (isNaN(document.getElementById("join").value) == false && parseInt(document.getElementById("join").value) >= 10000000 && parseInt(document.getElementById("join").value) <= 99999999) {
      db.collection("chat").doc(document.getElementById("join").value).get()
        .then((doc) => {
          if (doc.exists) {
  ...
  function commands(command) {
    if (command.includes('!cmd/ban ')) {
      ...
      //   if (doc.data().includes(banUser)) {
          if (!banUser.includes('☆')) {
            db.collection('ban').doc(banUser).set({
              banned: true
            })
            .then(() => {
    ...
    if (command.includes('!cmd/delete ')) {
      ...
      if (deleteVal == "chat" && window.location.search.replace("?id=", "") != "10000000") {
        docRef.delete().then(() => { window.location.href = '.'; });
      } else {
    ...
    if (Notification.permission != "granted") {
      Notification.requestPermission().then(function (permission) {
        if (permission == "granted") {
          var notification = new Notification("Chat for School", { body: "Hi there! Welcome to Chat for School!" });
        }
      });
    }
  }
